...
app.use(cors()); // تفعيل CORS لكل الدومينات
...
app.get("/price", async (req, res) => {
  try {
    ...
    res.json({ coin: symbol, price_usd: response.data[symbol].usd });
...
app.get("/signal", async (req, res) => {
  try {
    ...
    if (!prices || prices.length < 20) {
      return res.status(400).json({ error: "Not enough data for signal" });
    }
